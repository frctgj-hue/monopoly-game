...
const Board: React.FC<BoardProps> = ({ board, players, onCellClick }) => {
  ...
  const renderCell = (property: Property, index: number) => {
    ...
    return (
      <div
        ...
        style={isOwned && owner ? { borderColor: owner.color, borderWidth: '3px' } : {}}
      ...
      >
        ...
        <div className={`${isCorner ? 'text-[11px]' : 'text-[8px]'} font-black text-center uppercase leading-tight text-gray-900 px-0.5`}>
          {property.name}
        </div>
  ...
  // Разделяем поле на 4 стороны
  const bottom = board.slice(0, 11);
  const left = board.slice(11, 20);
  const top = board.slice(20, 31).reverse();
  const right = board.slice(31, 40).reverse();
    ...
    <div className="w-[1000px] h-[1000px]">
      <div className="board-container relative shadow-2xl rounded-lg overflow-hidden bg-[#CDE6D0] w-full h-full border-4 border-black" style={{
        ...
        <div className="grid grid-cols-11 gap-0 text-xs w-full h-full">
          {/* Верхняя сторона */}
          {top.map((property, idx) => renderCell(property, 30 - idx))}

          {/* Средняя часть */}
          <div className="col-span-11 row-span-9 grid grid-cols-11">
            {/* Левая сторона */}
            <div className="grid grid-rows-9">
              {left.map((property, idx) => renderCell(property, 11 + idx))}
            </div>
            ...
            {/* Правая сторона */}
            <div className="grid grid-rows-9">
              {right.map((property, idx) => renderCell(property, 39 - idx))}
            </div>
          </div>

          {/* Нижняя сторона */}
          {bottom.map((property, idx) => renderCell(property, idx))}
        </div>
