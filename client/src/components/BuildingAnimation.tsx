    ...
    if (houses === 5) {
      ...
      return (
        <div className={`relative inline-block ${animatingHouse === 4 ? 'animate-build-house' : ''}`}>
          {/* Красный кружок */}
          <div className="w-6 h-6 bg-red-600 rounded-full border-2 border-black flex items-center justify-center" style={{ boxShadow: '2px 2px 0px rgba(0,0,0,0.3)' }}>
            {/* Зеленый дом внутри */}
            <div className="w-3 h-3 bg-[#1FB25A] border border-black relative" style={{ boxShadow: '1px 1px 0px rgba(0,0,0,0.2)' }}>
              <div className="absolute -top-1 left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-[4px] border-r-[4px] border-b-[4px] border-l-transparent border-r-transparent border-b-[#1FB25A]"></div>
            </div>
          </div>
          {showSparkles && (
